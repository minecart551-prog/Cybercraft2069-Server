...
function interact(event) {
    ...
    if (g) {
        ...
        // Give bomb if: player is criminal AND (bomb is waiting at lobby OR refresh timer has elapsed)
        if (pIdx >= 0 && players[pIdx].team === TEAM_CRIMINAL) {
            var bombReady = g.bombAvailableAtLobby || checkBombRefresh()
            if (bombReady) {
                doBombRefresh() // strip any existing bomb from all criminals first
                g.bombAvailableAtLobby = false
                g.bombRefreshTime = SYS.currentTimeMillis() // always start a FRESH full countdown
...
function customGuiButton(event) {
    ...
    if (bid === L.BTN_JOIN_CRIMINAL) {
        ...
        var timerNotStarted = (!g.activeStartTime || g.activeStartTime === 0)
        ...
        // Always set a fresh activeStartTime if not already running
        if (timerNotStarted) {
            g.activeStartTime = SYS.currentTimeMillis()
        }
        setPlayers(g, players); saveGame(g)
        player.message("\u00a7c[Team] Joined Criminal! You will be rewarded \u00a7e" + fmt(DAILY_CONTRIBUTION) + " \u00a7cas team contribution per Minecraft day.")
        try { player.setSpawnpoint(CRIMINAL_RESPAWN.x, CRIMINAL_RESPAWN.y, CRIMINAL_RESPAWN.z) } catch(e) {}
        // Give bomb directly only if this is the very first criminal
        if (firstCriminal) {
            g.bombAvailableAtLobby = false
            g.bombRefreshTime = SYS.currentTimeMillis() // start a FRESH full countdown now
            saveGame(g)
            try { player.giveItem(player.getWorld().createItem(BOMB_ITEM_ID, 1)) } catch(e) {}
            player.message("\u00a7cYou received the bomb! Plant it at a bomb site!")
            teamBroadcast("\u00a7c[Bomb] First criminal joined via lobby! Bomb given - plant it!")
        } else {
            player.message("\u00a7eThe bomb is already in play. Coordinate with your team!")
        }
        openLobbyGui(event); return
    }
