...
// LOBBY SPAWN
// ============================================================================
var LOBBY_SPAWN = { x: 2464, y: 47, z: 853 } 
...
// ============================================================================
var BOMB_TIMER_SECONDS     = 360
var DEFUSE_TIME_WITH_KIT   = 5
var DEFUSE_TIME_NO_KIT     = 10
var ROUND_END_DELAY        = 5
var BOMB_REFRESH_MINUTES   = 6  // How often bomb refreshes (clears and re-enables pickup)
var MINECRAFT_DAY_MS       = 24000 * 50  // 1 Minecraft day = 24000 ticks * 50ms/tick = 20 real minutes
var DAILY_CONTRIBUTION     = 100  // cents added per Minecraft day ($1.00)
...
// ============================================================================
var POLICE_RESPAWN   = { x: 2374, y: 43, z: 951 }
var CRIMINAL_RESPAWN = { x: 2498, y: 42, z: 903 }
...
function doBombRefresh() {
    var g = getGame()
    if (!g) return
    // Mark bomb as ready (sentinel -1). The lobby block sets a fresh timestamp when it gives the bomb.
    g.bombRefreshTime = -1
...
// ============================================================================
// DAILY CONTRIBUTION TICK
// ============================================================================
// Call once per player timer tick. Checks if a full Minecraft day has elapsed
// since the player's lastDayTick and, if so, adds DAILY_CONTRIBUTION to their
// contribution and their team's fund. Returns true if contribution was awarded.
function tickDailyContribution(player, g) {
    if (!g || !player) return false
    var uuid = player.getUUID()
    var players = getPlayers(g)
    var pIdx = -1
    for (var i = 0; i < players.length; i++) if (players[i].uuid === uuid) { pIdx = i; break }
    if (pIdx < 0) return false  // not in game
    var p = players[pIdx]
    if (!p.lastDayTick || p.lastDayTick <= 0) return false  // no timer set yet
    var now = SYS.currentTimeMillis()
    if (now - p.lastDayTick < MINECRAFT_DAY_MS) return false  // day not over yet
    // Award contribution
    p.contributed += DAILY_CONTRIBUTION
    p.lastDayTick = now  // reset for next day
    if (p.team === TEAM_POLICE) g.policeFund += DAILY_CONTRIBUTION
    else if (p.team === TEAM_CRIMINAL) g.criminalFund += DAILY_CONTRIBUTION
    setPlayers(g, players)
    saveGame(g)
    player.message("\u00a7a[Team] Rewarded " + DAILY_CONTRIBUTION + "\u00a2 to your team contribution. \u00a77(Total: " + fmt(p.contributed) + ")")
    return true
}
