...
function timer(event) {
    // Daily contribution runs on its own slower timer
    if (event.id === dailyContribTimerId) {
        var player = event.player
        openPlayers[player.getUUID()] = { player: player, API: event.API }
        var g = getGame()
        if (g) tickDailyContribution(player, g)
        return
    }
    ...
    if (g.phase === PHASE_ACTIVE) {
        var rt = g.bombRefreshTime
        if (rt > 0) {
            // Timer is actively running - show countdown from its actual start
            var elapsed = Math.floor((SYS.currentTimeMillis() - rt) / 1000)
            bombDeadline = Math.max(0, BOMB_REFRESH_MINUTES * 60 - elapsed)
        } else if (rt === -1) {
    ...
    if (g.phase === PHASE_ACTIVE) {
        ...
        if (p.team === TEAM_CRIMINAL) {
            if (g.bombAvailableAtLobby) {
                ...
                phaseMsg = "§cReturn to lobby at 2498, 42, 903 to get bomb!"
...
function openTeamPCGui(event) {
    ...
    gui.addButton(70, "\u2139 Tutorial", 280, 12, 90, 15)
    var policeTotal = 0
    var criminalTotal = 0
...
function endBombTeamRound(winner, reason) {
    ...
    // Build a map of online players from the world (reliable, not openPlayers)
    var onlinePlayerMap = {}
    var world = getGameWorld()
    if (world) {
        try {
            var allWorldPlayers = world.getAllPlayers()
            for (var wi = 0; wi < allWorldPlayers.length; wi++) {
                var wp = allWorldPlayers[wi]
                try { onlinePlayerMap[wp.getUUID()] = wp } catch(e) {}
            }
        } catch(e) {}
    }
    ...
    for (var i = 0; i < players.length; i++) {
        ...
        if (p.team === winner) {
            ...
            var onlinePlayer = onlinePlayerMap[p.uuid]
            if (onlinePlayer) {
                // Player is online - pay directly
                giveCoins(onlinePlayer, reward)
                onlinePlayer.message("§a+ " + fmt(reward) + " (win payout: " + Math.round(pct * 100) + "% share + your contribution)")
            } else {
