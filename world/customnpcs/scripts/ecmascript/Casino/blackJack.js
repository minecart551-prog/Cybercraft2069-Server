...
// ============================================================================
function removeGuiComponents(gui, playerMaxCid) {
    ...
    var endCid = (playerMaxCid !== undefined && playerMaxCid !== null) ? playerMaxCid : maxCid
    for (var d = 9000; d < endCid; d++) {
...
function openGui(event) {
    ...
    openPlayers[uuid] = { player: player, API: api, gui: gui, maxCid: maxCid }
}
...
function refreshAllGuis(excludeUuid) {
    ...
    for (var i = 0; i < uuids.length; i++) {
        ...
        if (entry && entry.player && entry.API && entry.gui) {
            try {
                removeGuiComponents(entry.gui, entry.maxCid)
                resetCid()
