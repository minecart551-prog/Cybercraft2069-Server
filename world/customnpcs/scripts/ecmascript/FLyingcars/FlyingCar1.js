...
// Key controls (W/A/S/D/Space/V) are handled by the Player Script: "flyingcar - key_controls.js"
...
var FUEL_CONSUME_INTERVAL = 400; // ticks (1200 = 1 minute)
...
function showPurchaseGUI(player, npc, api) {
    ...
    var localGui = api.createCustomGui(200, 120, 0, false, player);
    localGui.addLabel(101, "§l§nCar for Sale", 100, -50, 1.0, 1.0);
    localGui.addLabel(102, "§7Price: §a" + CAR_PRICE_AMOUNT + "x §f" + CAR_PRICE_ITEM, 100, -30, 0.9, 0.9);
    localGui.addButton(10, "§aPurchase", 60, 10, 80, 20);
    localGui.addButton(11, "§cClose", 60, 40, 80, 20);
...
function showOwnerGUI(player, npc, api, carData) {
    ...
    var localGui = api.createCustomGui(350, 260, 0, false, player);

    localGui.addLabel(101, "§l§nYour Car", 120, -125, 1.0, 1.0);

    // Fuel slot (top left)
    localGui.addLabel(130, "§6⛽ Fuel:", 21, 28, 0.8, 0.8);
    fuelSlot = localGui.addItemSlot(21, 38);
    ...
    localGui.addButton(21, "§eSell Car", 19, -10, 60, 18);

    // Right column: Members list
    localGui.addLabel(102, "§eMembers:", 250, -70, 0.9, 0.9);

    // Assign to guiRef before renderMemberList so it can reference it
    guiRef = localGui;

    // Build member list before showing
...
function showViewerGUI(player, npc, api, carData) {
    ...
    if (isMember(npc, playerName)) {
        localGui = api.createCustomGui(240, 130, 0, false, player);
        localGui.addLabel(101, "§l§nCar Storage", 120, -65, 1.0, 1.0);
        localGui.addLabel(130, "§6⛽ Fuel:", 21, 28, 0.8, 0.8);
        fuelSlot = localGui.addItemSlot(21, 39);
        var fuelAmt = getFuelAmount(npc);
        if (fuelAmt > 0) {
            try { fuelSlot.setStack(player.world.createItem(FUEL_ITEM, Math.min(fuelAmt, 100))); } catch(e) {}
        }
        localGui.showPlayerInventory(0, 43, false);
        localGui.addButton(11, "§7Close", 85, 50, 50, 16);
    } else {
        localGui = api.createCustomGui(220, 120, 0, false, player);
        localGui.addLabel(101, "§l§nCar Info", 110, -50, 1.0, 1.0);
        localGui.addLabel(102, "§7Owner: §f" + (carData.owner || "None"), 110, -30, 0.9, 0.9);
        localGui.addLabel(130, "§6⛽ Fuel:", 21, 28, 0.8, 0.8);
        fuelSlot = localGui.addItemSlot(21, 39);
        var fuelAmt = getFuelAmount(npc);
        if (fuelAmt > 0) {
        ...
        localGui.addButton(11, "§7Close", 85, 30, 50, 16);
    }

    guiRef = localGui; // assign globally only after fully built
    player.showCustomGui(guiRef);
}
...
function isKeyHeld(npc, keyCode) {
    return npc.getStoreddata().get("key_" + keyCode) === "true";
}
